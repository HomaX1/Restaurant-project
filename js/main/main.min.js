var modalWrap=".modal-wrap",galleryFunctions={showModalImg:function(){$(".gallery-products").on("click",function(a){"gallery-product"===a.target.parentElement.className&&($(modalWrap).css("display","flex"),$(".modal-product__img").attr("src",a.target.currentSrc||a.target.src))})},closeModalImg:function(){$(".modal-product__icon-cancel").on("click",function(){$(modalWrap).hide()}),$(document).mouseup(function(a){0===$(modalWrap).has(a.target).length&&$(modalWrap).hide()})},renderModalImg:function(){galleryFunctions.showModalImg(),galleryFunctions.closeModalImg()}};
...
var sortInfo=".sorts-info",sortText=".sorts-info__text",sortList=".sorts-list",menuFunctions={listShow:function(t){$(sortInfo).click(function(){$(sortList).fadeToggle()}),menuFunctions.selectOption(t)},selectOption:function(t){$(sortList).click(function(e){$(sortList).fadeOut();var n=$(e.target).text();switch(n){case"from cheap to expensive":t.sort(menuFunctions.increaseSort),menu.innerHTML=MyApp.templates.menu({menu:t}),$(sortText).text(n),menuFunctions.listShow(t);break;case"from expensive to cheap":t.sort(menuFunctions.decreaseSort),menu.innerHTML=MyApp.templates.menu({menu:t}),$(sortText).text(n),menuFunctions.listShow(t)}}),$("body").mouseup(function(t){0===$(".sorts").has(t.target).length&&$(sortList).fadeOut()})},increaseSort:function(t,e){var n=parseFloat(t.price),o=parseFloat(e.price);return n>o?1:n<o?-1:void 0},decreaseSort:function(t,e){var n=parseFloat(t.price),o=parseFloat(e.price);return n>o?-1:n<o?1:void 0}};
var tableNumberInput="#table-number",formSelect=".form-select",selectFrom=".form-select-from",selectTo=".form-select-to",formSelectList=".form-select-list",formSelectListFrom=".form-select-list-from",formSelectListTo=".form-select-list-to",validName="#valid-name",validPhone="#valid-phone",validTableNumber="#valid-table-number",formInput=".form-reservation__input",successSubmit=".form-reservation__text",selectTextFrom=".form-select__text-from",selectTextTo=".form-select__text-to",firstName="#first-name",phoneNumber="#phone-number",reservationFunctions={showFormList:function(){$(formSelect).click(function(){$(this).hasClass("form-select-from")?($(formSelectListTo).fadeOut(),$(formSelectListFrom).fadeToggle()):($(formSelectListFrom).fadeOut(),$(formSelectListTo).fadeToggle())}),$(document).mouseup(function(e){0===$(formSelect).has(e.target).length&&$(formSelectList).fadeOut()})},selectOptionList:function(){$(formSelectList).click(function(e){var t="";$(e.target).hasClass("disabled-time")||($(e.target.parentElement).hasClass("form-select-list-from")?($(successSubmit).hide(),$(formSelectList).fadeOut(),t=e.target.innerText,$(selectTextFrom).text(t),$(selectFrom).removeClass("border")):($(successSubmit).hide(),$(formSelectList).fadeOut(),t=e.target.innerText,$(selectTextTo).text(t),$(selectTo).removeClass("border")))})},numberTransfer:function(){$(".scheme-tables").on("click",function(e){if($(successSubmit).hide(),$(validTableNumber).hide(),$(tableNumberInput).removeClass("border"),$(e.target).hasClass("scheme-tables__item")&&!$(e.target).hasClass("booked-table")){var t=+$(e.target).attr("data-id");$(tableNumberInput).val(t)}if("IMG"!==e.target.tagName&&$(e.target).attr("data-times").length>0){var s=JSON.parse($(e.target).attr("data-times"));$(".form-select-list__item").removeClass("disabled-time"),$(".form-select-list-from li").each(function(e,t){var o=parseInt($(t).text());s.forEach(function(e){for(var s=e.timeFrom;s<e.timeTo;s++)o===s&&$(t).addClass("disabled-time")})}),$(".form-select-list-to li").each(function(e,t){var o=parseInt($(t).text());s.forEach(function(e){for(var s=e.timeFrom+1;s<=e.timeTo;s++)o===s&&$(t).addClass("disabled-time")})})}})},validation:function(){$(formInput).keyup(function(e){$(successSubmit).hide();switch(e.target.id){case"first-name":if(""!==$(this).val()){/^[A-Za-z]{1,50}$/.test($(this).val())?($(this).removeClass("border"),$(validName).hide()):($(this).addClass("border"),$(validName).text("You can use latin letters only!").show())}break;case"phone-number":if(""!==$(this).val()){/^\d{3}[\-]\d{3}[\-]\d{4}$/.test($(this).val())?($(this).removeClass("border"),$(validPhone).hide()):($(this).addClass("border"),$(validPhone).text("Enter the phone number as in the example!").show())}}})},getBookedTables:function(){$.get("/tables.json",function(e){$(".scheme-tables__item").each(function(t,s){var o=e.filter(function(e){return e.tableNumber===+$(s).attr("data-id")});!1!==o[0].booking&&0!==o[0].booking.length?$(s).attr("data-times",JSON.stringify(o[0].booking)):$(s).addClass("booked-table")})})},sendData:function(){$(".form-reservation__button").on("click",function(e){if(e.preventDefault(),""===$(firstName).val()&&($(firstName).addClass("border"),$(validName).text("Please, enter your name!").show()),""===$(phoneNumber).val()&&($(phoneNumber).addClass("border"),$(validPhone).text("Please, enter your phone number!").show()),""===$(tableNumberInput).val()&&($(tableNumberInput).addClass("border"),$(validTableNumber).text("Click on the table you want to book on the picture!").show(),reservationFunctions.numberTransfer()),"none"===$(selectTextFrom).text()&&$(selectFrom).addClass("border"),"none"===$(selectTextTo).text()&&$(selectTo).addClass("border"),""!==$(formInput).val()&&""!==$(tableNumberInput).val()&&"none"!==$(selectTextFrom).text()&&"none"!==$(selectTextTo).text()){$(successSubmit).fadeIn();var t={firstName:$(firstName).val(),phoneNumber:$(phoneNumber).val(),tableNumber:$(tableNumberInput).val(),timeFrom:$(selectTextFrom).text(),timeTo:$(selectTextTo).text()};console.log(t)}})},renderReservation:function(){reservationFunctions.getBookedTables(),reservationFunctions.showFormList(),reservationFunctions.selectOptionList(),reservationFunctions.numberTransfer(),reservationFunctions.validation(),reservationFunctions.sendData()}};